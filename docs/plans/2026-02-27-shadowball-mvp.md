# Shadowball MVP Implementation Plan
...
**Files:**
- Create: `shadowball/` (프로젝트 루트)

**Step 1: 프로젝트 생성**
```bash
cd /root/.openclaw/workspace/projects
npx create-next-app@latest shadowball \
...
**Step 6: Commit**
```bash
git init && git add . && git commit -m "feat: init shadowball next.js project"
```
...
export function DarkraiHeader() {
  return (
    <header className="flex items-center gap-4 py-6 px-4 border-b border-gray-800">
      ...
      />
      <div>
        <h1 className="text-2xl font-mono font-bold text-white tracking-tight">Shadowball</h1>
...
**Step 1: GitHub 레포 생성**
```bash
gh repo create dalbee-ship-it/shadowball --public --source=. --push
```
...
import httpx
httpx.post(
  "https://shadowball.vercel.app/api/tasks",
  json={
